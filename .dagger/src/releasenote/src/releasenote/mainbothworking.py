...
class Releasenote:
    ...
    async def check_and_release(
        ...
        # -----------------------------
        # 4. Detect version from pyproject.toml or package.json
        # -----------------------------
        ...
        if not feat_v:
            return f"❌ No valid version found on branch {source_branch}. Skipping release."

        if not is_valid_semver(feat_v):
            return f"❌ Invalid semantic version detected: {feat_v}. Expected MAJOR.MINOR.PATCH."

        if feat_v == prod_v:
            return (
                f"✅ No release needed. {source_branch} version {feat_v} "
                f"matches {default_prod_branch} version {prod_v}."
            )
        ...
        # -----------------------------
        # 6. Push release branch + tag (idempotent + safe)
        # -----------------------------
        await (
            container
            ...
            # Create or sync release branch (fast-forward only)
            .with_exec([
                "sh", "-c",
                f"""
                if git ls-remote --heads origin {new_branch} | grep {new_branch}; then
                    echo 'Branch {new_branch} exists. Syncing...';

                    git fetch origin {new_branch}:{new_branch};
                    git checkout {new_branch};

                    git pull --ff-only origin {new_branch} || \
                        echo 'Fast-forward not possible. Continuing without merge.';
                else
            ...
            .with_exec([
                ...
                "else "
                f"git commit -m 'chore: release {feat_v}'; "
                "fi"
            ])
        ...
        result = {
            ...
            "tag": new_tag,
            "message": "Release branch and tag processed successfully.",
        }
